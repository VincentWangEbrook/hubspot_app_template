...
export default function ContactsPage() {
  ...
  const handleContactClick = (contact: Contact) => {
    router.push(`/${tenantId}/hubspot/contacts/${contact.hubspot_id}`);
  };

  const getLineBoundCount = () => {
    return contacts.filter(c => c.line_user_id).length;
  };
                ...
                >
                  <div className="p-6">
                      ...
                      {contact.line_display_name && (
                        <div className="flex items-center gap-2">
                          <span className="px-2 py-1 bg-green-100 dark:bg-green-900/30 
                            ...
                            LINE: {contact.line_display_name}
                          </span>
                        </div>
                      )}
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 pt-4 
                      ...
                      <Calendar size={14} />
                      <span>创建于 {formatDate(contact.hubspot_created_at)}</span>
                    </div>
                  </div>
                </div>
              ))}
