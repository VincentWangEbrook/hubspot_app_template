...
// 配置常量（集中管理，便于维护）
const CONFIG = {
  PROTECTED_ROUTES: ['/reports', '/admin', '/settings', '/system', '/tenant', '/:tenantId/hubspot', '/:tenantId/inbox'], // 需要登录的路由前缀
...
export async function proxy(req: NextRequest) {
  ...
  const isTenantInboxRoute = /^\/[^/]+\/inbox/.test(pathname);

  // 如果不是受保护路由且不是租户Hubspot路由，则放行
  if (!isProtectedRoute && !isTenantHubspotRoute && !isTenantInboxRoute) {
